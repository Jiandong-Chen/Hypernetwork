...
data = pd.read_csv("./data/Tabular/WEC_Perth_49.csv")
df = data
X = df.drop('Total_Power', axis = 1) #feature
X = X.filter(regex=r'^[XY]\d+$')
y = df['Total_Power'] #target

st=StandardScaler()
X_scaled=st.fit_transform(X)

scaler_y = StandardScaler()
y_scaled = scaler_y.fit_transform(y.values.reshape(-1, 1))  # Standardize target

X_tensor = torch.tensor(X_scaled, dtype=torch.float32)  # Input features
y_tensor = torch.tensor(y_scaled, dtype=torch.float32).squeeze()  # Target values
...
# model = MLP(input_dim = X.shape[1], num_layers=3, output_dim = 1)
# model = MainMLP(input_dim = X.shape[1], hidden_dim = 100, output_dim = 1)
...
# Save the plots
plt.tight_layout()
plt.savefig('./figures/Tabular/WEC_hyper.png')